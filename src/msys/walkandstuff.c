...
descendonce(unsigned long *pgtbl, char *va, unsigned int level)
{
	...
	if(level > 2)
		ultimateyeet("descend: bad level");
	...
	if(!pte) return NULL;
	else return (unsigned long *)physviapte(*pte);
}
...
pteforva(unsigned long *pgtbl, char *va)
{
	...
	if(!pgtbl)
		ultimateyeet("translate: bad pgtbl");
	if(!va || (unsigned long)va >= MAXVA)
		ultimateyeet("translate: bad va");
